...
function get_google_reviews( $place_id ) {
	...
	*/
	...
	$place_details_response = wp_remote_get( esc_url_raw( $place_details_url ) );
	...
	<h2 class="heading">
		...
		if ( $logo_icon ) {
			?>
			<span class="icon"><img src="<?php echo esc_url( $logo_icon ); ?>" alt="<?php echo esc_attr( $place_name ); ?> Logo"></span><?php } ?>
		<?php echo esc_html( $place_name ); ?> Google口コミレビュー
	</h2>
	<p class="vicinity"><?php echo esc_html( $address ); ?></p>
	<p class="rating"><span class="number"><?php echo esc_html( $rating ); ?></span> <span class="rating-star"><?php echo wp_kses_post( generate_star_rating( $rating ) ); ?></span> <a href="https://search.google.com/local/reviews?placeid=<?php echo esc_attr( $place_id ); ?>" target="_blank" rel="nofollow" class="count"><?php echo esc_html( $review_count ); ?> reviews</a></p>
		<?php
	...
	<ul class="listing">
			...
			foreach ( $reviews_list as $index => $review ) :
				...
				$comment                   = $review['text'];
				?>
		<li class="rating-<?php echo esc_attr( $rating ); ?>" data-index="<?php echo esc_attr( $index ); ?>">
			...
			<span class="review-meta">
				<span class="author-name">
						<a href="<?php echo esc_url( $author_url ); ?>" target="_blank" rel="nofollow"><?php echo esc_html( $author ); ?></a>
					</span>
				<span class="rating">
					<?php echo wp_kses_post( generate_star_rating( $rating ) ); ?>
				</span>
				<span class="relative-time-description"><?php echo esc_html( $relative_time_description ); ?></span>
			</span>
			<div class="text">
...
<div id="google-business-reviews-rating" class="google-business-reviews-rating gmbrr contrast stars-css">
	...
	<div class="review-link">
		<a href="https://search.google.com/local/reviews?placeid=<?php echo esc_attr( $place_id ); ?>" class="button" target="_blank" rel="nofollow">もっとみる ＞＞</a>
	</div>
</div>
	<?php
}
